...
public final class NostrRelayManager: ObservableObject {
    public static let shared = NostrRelayManager()
    // Track gift-wraps (kind 1059) we initiated so we can log OK acks at info
    public private(set) static var pendingGiftWrapIDs = Set<String>()
    ...
    @Published public private(set) var relays: [Relay] = []
    ...
    /// Relay set for public geohash traffic: nearest geo relays plus defaults when allowed.
    public func publicGeohashRelayURLs(for geohash: String, count: Int = TransportConfig.nostrGeoRelayCount) -> [String] {
        let geoRelays = GeoRelayDirectory.shared.closestRelays(toGeohash: geohash, count: count)
        return allowedRelayList(from: geoRelays + Self.defaultRelays)
    }
    ...
    public func sendEvent(_ event: NostrEvent, to relayUrls: [String]? = nil) {
        ...
        let requestedRelays = relayUrls ?? defaultDMRelayURLs()
    ...
    public func subscribe(
        ...
        do {
            ...
            let baseUrls = relayUrls ?? defaultDMRelayURLs()
    ...
    private func applyDefaultRelayPolicy(force: Bool = false) {
        let shouldAllow = hasMutualFavorites || hasLocationPermission || FavoritesPersistenceService.shared.hasAnyNostrFavorite
    ...
    private func defaultDMRelayURLs() -> [String] {
        let configured = relays.map(\.url)
