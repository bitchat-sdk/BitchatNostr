...
public struct NostrEmbeddedBitChat {
    /// Build a `bitchat1:` base64url-encoded BitChat packet carrying a private message for Nostr DMs.
    public static func encodePMForNostr(content: String, messageID: String, recipientPeerID: PeerID, senderPeerID: PeerID) -> String? {
    ...
    public static func encodeAckForNostr(type: NoisePayloadType, messageID: String, recipientPeerID: PeerID, senderPeerID: PeerID) -> String? {
    ...
    public static func encodeAckForNostrNoRecipient(type: NoisePayloadType, messageID: String, senderPeerID: PeerID) -> String? {
        guard type == .delivered || type == .readReceipt else { return nil }
    ...
    public static func encodePMForNostrNoRecipient(content: String, messageID: String, senderPeerID: PeerID) -> String? {
        let pm = PrivateMessagePacket(messageID: messageID, content: content)
        guard let tlv = pm.encode() else { return nil }
    ...
    /// Build a `bitchat1:` base64url-encoded BitChat packet carrying a file transfer for Nostr DMs.
    public static func encodeFileTransferForNostr(
        filePacket: BitchatFilePacket,
        recipientPeerID: PeerID,
        senderPeerID: PeerID
    ) -> String? {
        guard let tlv = filePacket.encode() else { return nil }

        var payload = Data([NoisePayloadType.fileTransfer.rawValue])
        payload.append(tlv)
